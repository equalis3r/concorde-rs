...
/*      -see the description of CCedgegen_x_nearest_neighbor_tour above     */
/*                                                                          */
...
static void add_to_list_and_reset(int *list, int *lcount, shortedge *nearlist,
    ...
    insert(int n, int m, shortedge *nearlist, CCdatagroup *dat, double *wcoord),
    x_quicksort(int *list, double *x, int l, int u);
static int run_x_k_nearest(int ncount, int num, CCdatagroup *dat,
                           double *wcoord, int wantlist, int *ecount,
                           int **elist, int doquad, int silent),
...
int CCedgegen_x_k_nearest(int ncount, int num, CCdatagroup *dat, double *wcoord,
                          int wantlist, int *ecount, int **elist, int silent) {
    return run_x_k_nearest(ncount, num, dat, wcoord, wantlist, ecount, elist, 0,
                           silent);
}
...
int CCedgegen_x_quadrant_k_nearest(int ncount, int num, CCdatagroup *dat,
                                   double *wcoord, int wantlist, int *ecount,
                                   int **elist, int silent) {
    return run_x_k_nearest(ncount, num, dat, wcoord, wantlist, ecount, elist, 1,
                           silent);
}
        ...
        if (usex) {
            if (doquad) {
                if (CCedgegen_x_node_quadrant_k_nearest(&xn, n, num, ncount,
                                                        list)) {
                    fprintf(stderr,
                            "CCedgegen_x_node_quadrant_k_nearest failed\n");
                    rval = 1;
                    goto CLEANUP;
                }
            } else {
...
int CCedgegen_x_node_quadrant_k_nearest(CCxnear *xn, int ni, int nearnum,
    ...
    if ((norm & CC_NORM_SIZE_BITS) == CC_D3_NORM_SIZE) {
        ...
        for (j = n - 1; j >= 0 && dtrunc((xn->dat.x[n] - xn->dat.x[j]) *
                                         scale) < nearlist[0].length;
             --j) {
            if (xn->dat.y[j] <= ny && xn->dat.z[j] <= nz)
                insert(n, j, nearlist, &(xn->dat), xn->w);
        }
        add_to_list_and_reset(list, &ntotal, nearlist, nearnum, xn->nodenames);
        for (j = n - 1; j >= 0 && dtrunc((xn->dat.x[n] - xn->dat.x[j]) *
                                         scale) < nearlist[0].length;
             --j) {
            if (xn->dat.y[j] <= ny && xn->dat.z[j] >= nz)
        ...
        add_to_list_and_reset(list, &ntotal, nearlist, nearnum, xn->nodenames);
        for (j = n + 1; j < ncount && dtrunc((xn->dat.x[j] - xn->dat.x[n]) *
                                             scale) < nearlist[0].length;
             j++) {
            if (xn->dat.y[j] <= ny && xn->dat.z[j] <= nz)
                insert(n, j, nearlist, &(xn->dat), xn->w);
...
int CCedgegen_x_node_nearest(CCxnear *xn, int ncount, int ni, char *marks) {
    ...
    if (norm == CC_GEOGRAPHIC)
        ...
        scale = 1.0;
    for (j = n - 1;
         j >= 0 && dtrunc((xn->dat.x[n] - xn->dat.x[j]) * scale) < bestdist;
         --j) {
    ...
    for (j = n + 1;
         j < ncount && dtrunc((xn->dat.x[j] - xn->dat.x[n]) * scale) < bestdist;
         j++) {
        if (!marks[xn->nodenames[j]]) {
            thisdist = CCutil_dat_edgelen(n, j, &(xn->dat));
            if (xn->w)
...
int CCedgegen_junk_node_nearest(CCdatagroup *dat, double *wcoord, int ncount,
    ...
    if (wcoord) {
        for (j = n - 1; j >= 0; j--) {
            if (!marks[j]) {
                thisdist =
                    CCutil_dat_edgelen(n, j, dat) + (wcoord[n] + wcoord[j]);
                if (thisdist < bestdist) {
                    bestdist = thisdist;
                    bestnode = j;
                }
...
int CCedgegen_x_greedy_tour(int ncount, CCdatagroup *dat, int *outcycle,
    ...
    if (perm == (int *)NULL || elen == (int *)NULL) {
...
int CCedgegen_x_qboruvka_tour(int ncount, CCdatagroup *dat, int *outcycle,
    ...
    if (deg == (int *)NULL || adj == (int **)NULL || adjspace == (int *)NULL) {
        fprintf(stderr, "Out of memory in CCedgegen_x_greedy_tour\n");
        rval = 1;
        goto CLEANUP;
    }
...
int CCedgegen_junk_nearest_neighbor_tour(int ncount, int start,
                                         CCdatagroup *dat, int *outcycle,
                                         double *val, int silent) {
    double len;
    int i, current, next;
    char *marks;
    ...
    */
    ...
    for (i = 1; i < ncount; i++) {
        marks[current] = 1;
        next = CCedgegen_junk_node_nearest(dat, (double *)NULL, ncount, current,
                                           marks);
        if (outcycle != (int *)NULL)
            outcycle[i] = next;
        len += (double)CCutil_dat_edgelen(current, next, dat);
...
int CCedgegen_junk_greedy_tour(int ncount, CCdatagroup *dat, int *outcycle,
                               double *val, int ecount, int *elist,
                               int silent) {
    int rval;
    int *perm = (int *)NULL;
    int *elen = (int *)NULL;
    ...
    while (tcount < ncount - 1) {
        marks[x] = 1;
        y = CCedgegen_junk_node_nearest(dat, (double *)NULL, ncount, x, marks);
        if (tcyc) {
            tcyc[2 * tcount] = x;
            tcyc[2 * tcount + 1] = y;
        }
